async function DeathNotify(body, connection, res) {
  ...
  if (idNguoiKhai === idNguoiChet) {
    res
      .status(202)
      .json({ status: 202, message: 'Người chết không thể tự khai tử' });
    return;
  }
      ...
      connection.query(checkQuery2, (error, result) => {
        if (error) {
          ...
          if ([...idNguoiThanList].indexOf(idNguoiKhai)) {
            res.status(202).json({
              status: 202,
              message: 'Không thể khai tử cho người không có quan hệ',
            });
            return;
          }
            ...
            (error, result) => {
              if (error) {
                if (error.errno === 1062) {
                  res.status(202).json({
                    status: 202,
                    message: 'Người này đã được khai tử trước đó',
                  });
                } else {
                  res.status(500).json({ error: 'Error' });
                }
              } else {
                // Update ghiChu in nhan_khau table
                const updateQuery = `UPDATE nhan_khau SET ghiChu = ? WHERE ID = ?`;
                connection.query(
                  updateQuery,
                  ['Dead', idNguoiChet],
                  (error, result) => {
                    ...
                    res
                      .status(200)
                      .json({ status: 200, message: 'Khai tử thành công.' });
                  },
                );
              }
            },
